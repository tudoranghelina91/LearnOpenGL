...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		int amountLocation = glGetUniformLocation(shader.ID, "amount");

		float amountValue;
		glGetUniformfv(shader.ID, amountLocation, &amountValue);

		if (amountValue < 0.2f && glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)
		{
			amountValue += 0.001;
		}

		if (amountValue > 0.0f && glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
		{
			amountValue -= 0.001;
		}

		shader.setFloat("amount", amountValue);
	}
